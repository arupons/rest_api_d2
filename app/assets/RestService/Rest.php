   ...
   private function limpiarEntrada($data) {
     ...
     if (is_array($data)) {
       ...
       if (get_magic_quotes_gpc()) {
         ...
         //Aunque actualmente se desaconseja su uso, muchos servidores tienen activada la extensi�n magic_quotes_gpc.
...
//         $this->datosPeticion = $this->limpiarEntrada($_POST);
   ...
   private function setData(){
       $verb = file_get_contents("php://input");
       $arr = $this->objectToArray(json_decode($verb));
       if(is_array($arr))
       {
           $this->datosPeticion = $arr;
       }else{
           parse_str($verb, $this->datosPeticion);
       }
